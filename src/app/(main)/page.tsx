      ...
      <div className="px-8 md:px-16 flex flex-col gap-16">
              ...
              {movies.map(movie => {
                ...
                return (
                  <Link key={movie.id} href={`/watch/movie/${movie.id}`} className="min-w-[240px] md:min-w-[280px] aspect-video relative group overflow-hidden rounded-xl cursor-pointer transition-all duration-500 hover:scale-[1.08] hover:z-30 hover-neon-glow border border-transparent">
                    ...
                    {hasDoubleSub && (
                      <span className="absolute top-3 left-3 z-20 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-extrabold px-2.5 py-1 rounded-full shadow-[0_2px_8px_rgba(99,102,241,0.5)] border border-blue-400/30">
                        ✨ Multi-Title (Çift Altyazı)
                      </span>
                    )}
        ...
        <section className="animate-fade-in-up delay-200">
          ...
          {series.length === 0 ? (
            ...
            <div className="flex gap-6 overflow-x-auto pb-8 pt-4 px-4 -mx-4 scrollbar-hide">
              {series.map(s => {
                const sEps = (db.episodes || []).filter(e => e.seriesId === s.id);
                const hasDoubleSub = sEps.some(e => e.subtitleTR && e.subtitleEN);
                return (
